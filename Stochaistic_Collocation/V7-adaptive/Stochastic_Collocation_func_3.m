...
% Test the algorithm
% for q = d:d+k
for filter_order = 1:1; q = 9;
%     [num_pts(q-1) pts] = Colloc_Pts(q);
%     err(q-1,1) = num_pts(q-1);
    [num_pts(filter_order) pts] = Colloc_Pts(q);
    err(filter_order,1) = filter_order;
    tic
...
%     A    = cheby_filter(length(smol)-1, filter_order);
%     smol = A*smol;
    toc
    
%     [err_loc err(q-1,2) err(q-1,3) err(q-1,4)] = error_analysis(smol, exact);
    [err_loc err(filter_order,2) err(filter_order,3) err(filter_order,4)] = error_analysis(smol, exact);
