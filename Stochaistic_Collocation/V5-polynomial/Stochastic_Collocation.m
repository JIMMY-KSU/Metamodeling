...
for analyt = 8:8, if  analyt ~= 3
global max1 min1 max2 min2

% Polynomial 'poly' or piecewise linear 'lin'
type = 'poly';
...
for q = 3:d+k
    ...
    if analyt == 4 %Special case for Zabaras Eq. 81
    ...
    for i = 1:test_pts
        for j = 1:test_pts
            ...
            if analyt == 4 %Special case for Zabaras Eq. 81
                ...
                smol(i,j)  = Smolyak_func(q, d, y, analyt, type);
            end
        end
    end
    toc
...
%     if q>d+1 && err_plt
%         error_plots(err(:,1), err(:,2), err(:,3), err(:,4), q)
%     end
