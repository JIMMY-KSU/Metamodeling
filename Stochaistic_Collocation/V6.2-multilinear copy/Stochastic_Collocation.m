...
z = Adaptive_Smolyak_func(d+j-1, d, f, range, reltol, abstol, x);
y = x;
for k = min_level+1:max_level
    q = d+k;
    for mek = 1:length(x)
        popp{mek} = unique(x{mek},'rows');
    end
    %Find heirarchical surpluses
    z = Adaptive_Smolyak_func(d+k-1, d, f, range, reltol, abstol, x);
...
vec_smol = SC_Interp(d, z.w, range, x_test(1,:), x_test(2,:));
n = 1;
for p = 1:test_pts
    for j = 1:test_pts
